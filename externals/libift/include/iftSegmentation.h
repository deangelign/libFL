 ...
 */
typedef enum {
    IFT_NONE_GRAD, IFT_IMAGE_BASINS, IFT_BRAIN_GRAD
} iftGradientAlg;
 ...
 * @ingroup Segmentation
 *
 * @param orien: Input image for the purpose of boundary orientation (e.g. a fuzzy object map)
 * @param basins: Input gradient image
 ...
 * @param orientation: Boundary orientation (0/1)
 ...
 */
  ...
  iftImage *iftOrientedWatershed(const iftImage *orien, const iftImage *basins, iftAdjRel *A, iftLabeledSet *seed, char orientation, iftSet *forbidden);
 ...
 */
iftImage  *iftAboveAdaptiveThreshold(iftImage *img, iftAdjRel *A, float perc, int niters, int value);
 ...
 * @return Threshold Image.
 */
iftImage  *iftBelowAdaptiveThreshold(iftImage *img, iftAdjRel *A, float perc, int niters, int value);
 ...
 * in the input image.
 */
iftImage *iftBorderImage(const iftImage *label);
