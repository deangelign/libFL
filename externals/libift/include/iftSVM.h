...
typedef struct ift_svm_linear_info{
  iftSample *w;
  float *rho;
  int nmodels;
  int nfeats;
 ...
 */
void iftSVMTrainOVO(iftSVM *svm, const iftDataSet *Z);
 ...
 * @param Z             Dataset used to train the SVM - it uses the samples with status TRAIN.
 * @param sample_status Status of the samples to be classified.
 * @return              Number of samples incorrectly predicted.
 */
 ...
 * @param Ztrain        Dataset used to train the SVM - it uses the samples with status TRAIN.
 * @param sample_status Status of the samples to be classified.
...
int iftSVMLinearClassifySampleOVA(const iftSVM *svm, iftSample *sample, uchar sample_status);

iftSVMLinearInfo *iftGetSVMLinearInfo(const iftSVM *svm);
void iftDestroySVMLinearInfo(iftSVMLinearInfo** svm_lin_info);
int iftSVMLinearFastClassifySampleOVA(const iftSVM *svm, iftSVMLinearInfo *svm_lin_info, iftSample *sample);

int iftSVMClassifyOVAGettingMCLUvalues(const iftSVM *svm, iftDataSet *Z, uchar sample_status);
int iftSVMClassifyOVAGettingMCLUvaluesFromSampleList(const iftSVM *svm, iftDataSet *Z, iftIntArray *sample_list);
