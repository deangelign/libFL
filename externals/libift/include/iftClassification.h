...
/**
 * @brief Trains a logistic regressor over the TRAIN samples in the dataset Z.
 * @author Peixinho
 * @date March 2016
 */
iftLogReg* iftLogRegTrain(iftDataSet* Z);
/**
 * @brief Classifies the TEST samples in the dataset Z, using the logistic regressor.
 * @author Peixinho
 * @date March 2016
 */
...
/**
 * @brief Classifies testing samples (status = TEST) from the dataset Ztest by using
 * Supervised OPF.
 ...
 * in the dataset.
 * @note If a testing sample is classified incorrectly, its status will be ERROR.
 * 
...
/**
 * Classifies testing samples (status = TEST) from the dataset Ztest by using
...
/** @brief This function performs OPF's semi-supervised algorithm using the
  * graph topology defined by the given MST. It returns a new set of
  * prototypes. In the first iteration of semi-supervised learning
  * prototype set 'S' should be all labeled samples. In the second
  * iteration 'S' should be the result of this function. Note that 'S'
  * is destroyed in the process. NOTE: I have to test this function
  * more thoroughly.
  *
  * @author Thiago V. Spina
  * @date October 23, 2015
*/
iftCplGraph* iftSemiSupTrain(iftDataSet *Zlabeled, iftDataSet *Zunlabeled, bool compute_dist_table);
